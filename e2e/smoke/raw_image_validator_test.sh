...
image="$(resolve_runfile "$1")" || {
    echo "consumer raw image is missing from runfiles" >&2
    exit 1
}
validator="$(resolve_runfile "$2")" || {
    echo "consumer raw image validator is missing from runfiles" >&2
    exit 1
}
python="$(resolve_runfile "$3")" || {
